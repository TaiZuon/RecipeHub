...
const HomePage = () => {
  ...
  const [searchParams, setSearchParams] = useState({ page: 1, size: 6, categoryType: [], title: "" });
  ...
  const fetchRecipes = async (page, categoryType = [], title = "") => {
    setLoading(true);
    try {
      const response = await getRecipes(page, 6, categoryType, title);
  ...
  useEffect(() => {
    fetchRecipes(page, searchParams.categoryType, searchParams.title);
  }, [searchParams, page]);
  ...
  const loadRecipes = async (page) => {
    ...
    setLoading(true);
    try {
      const response = await fetchRecipes(page, searchParams.categoryType, searchParams.title);
      const data = response?.data?.data || [];
