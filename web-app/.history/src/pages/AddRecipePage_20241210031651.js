...
const AddRecipePage = () => {
  ...
  const handleAddIngredient = async (ingredient.id) => {
    if (!selectedIngredients.includes(ingredient)) {
      setSelectedIngredients((prev) => [...prev, ingredient]); // Thêm nguyên liệu mới

      // Gọi API để tạo RecipeIngredient
      try {
        console.log(ingredient)
        const recipeIngredient = { ingredientId: ingredient.id, recipe: null }; // Thay đổi recipeId sau khi tạo công thức
