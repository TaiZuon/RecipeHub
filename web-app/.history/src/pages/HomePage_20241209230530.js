...
const HomePage = () => {
  ...
  const loadRecipes = async (searchParams.page, searchParams.title) => {
    console.log(`loading recipes for page ${searchParams.page}`);
    setLoading(true);
    try {
      const response = await getRecipes(searchParams.page, searchParams.title);
  ...
  useEffect(() => {
    loadRecipes(searchParams.page, searchParams.title);
  }, [searchParams]);
