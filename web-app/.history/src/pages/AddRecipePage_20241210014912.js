    ...
    <div style={{ display: "flex", gap: "5px", flexWrap: "wrap" }}>
      {ingredients.map((ingredient) => (
        <Button 
          ...
          onClick={() => {
            const currentValue = form.getFieldValue("ingredients") || ""; // Lấy giá trị hiện tại
            console.log(ingredient.nam)
            const newValue = currentValue 
              ? `${currentValue}, ${ingredient.name}` 
              : ingredient.name;
            form.setFieldsValue({ ingredients: newValue }); // Cập nhật giá trị
          }}
        >
          {ingredient.name}
        </Button>
