...
const AddRecipePage = () => {
  ...
  const onFinish = async (values) => {
    ...
    if (!token) {
      message.error("Bạn cần đăng nhập để thêm công thức!");
      return;
    }
    ...
    try {
      const response = await axios.post("http://localhost:8081/api/recipes", recipeRequest);
    ...
    <Layout>
      ...
      <Content style={{ padding: "20px", maxWidth: "600px", margin: "0 auto" }}>
        ...
        <Form name="add-recipe" onFinish={onFinish}>
          ...
          <Form.Item
            name="steps"
            rules={[{ required: true, message: "Hãy nhập các bước thực hiện!" }]}
          >
            <Input.TextArea placeholder="Các bước thực hiện (ngăn cách bằng dấu chấm)" rows={4} />
          </Form.Item>
          <Form.Item
            name="images"
