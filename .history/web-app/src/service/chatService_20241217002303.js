...
export const chatService = {
    ...
    deleteRoom: async (roomId, userId) => {
        try {
            const response = await axios.delete(`${API_BASE_URL}/api/chat/room/${roomId}`, null, {
                params: { userId }
            });
            return response.data;
        } catch (error) {
    ...
    sendFileMessage: async (roomId, userId, { file, type }) => {
        try {
            ...
            const response = await axios.post(`${API_BASE_URL}/api/chat/room/${roomId}/file-message`, formData, {
                params: { userId },
                headers: {
