...
const fetchPendingRecipes = async () => {
    try {
      setLoading(true);
      const response = await axios.get("http://localhost:8080/api/recipes/status", {
  ...
  const handleApprove = async (recipeId) => {
    try {
      await axios.post(`http://localhost:8080/api/recipes/${recipeId}/approve`);
  ...
  const handleReject = async (recipeId) => {
    try {
      await axios.post(`http://localhost:8080/api/recipes/${recipeId}/reject`);
